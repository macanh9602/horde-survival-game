    ...
    public static class Serializer
    {
        ...
        public static T XmlDeserializeFromResourses<T>(string fileName, bool logIfFileNotExists = true) where T : new()
        {
            ...
            else
            {
                if (logIfFileNotExists)
                {
                    Debug.LogError("File : \"" + fileName + "\" does not exist at Resources folder. Maybe file located at subsidiary folder or there is extention in the end of file NameGear.");
                }
                return new T();
            }
        }
        ...
        public static T JsonDeserializeFromPath<T>(string absolutePath, string secureKey = "", bool logIfFileNotExists = true) where T : new()
        {
            if (FileExistsAtPath(absolutePath))
            {
                ...
                using (StreamReader sr = new StreamReader(file))
                {
                    ...
                    //Debug.Log($"<color=green>json : {jsonObject}</color>");
                    try
                    {
        ...
        {
            string jsonObject = JsonUtility.ToJson(objectToSerialize, true);
            //string jsonObject = JsonConvert.SerializeObject(objectToSerialize);
            jsonObject = FormatJson(jsonObject, 2);
            //Debug.Log($"<color=green>JsonSerializeToPath</color>");
        ...
        /// </summary>
        private static string FormatJson(string jsonString, int indentSize)
        {
            var indent = 0;
            var quoted = false;
                ...
                switch (ch)
                {
                    ...
                    case '[':
                        ...
                        if (!quoted)
                        {
                            sb.AppendLine();
                            indent++;
                            sb.Append(new string(indentString[0], indent * indentSize));
                        }
                        break;
                    case '}':
                    case ']':
        ...
        {
            Debug.Log($"<color=#2884FB>{persistentDataPath + fileName}</color>");
