...
public class HexGridView : MonoBehaviour
{
    ...
    private readonly List<CellView> spawnedCells = new List<CellView>();
    public Dictionary<Vector3Int, CellView> viewDic = new();
    ...
    public void SpawnCells()
    {
        ...
        for (int row = 0; row < gridData.height; row++)
        {
            for (int col = 0; col < gridData.width; col++)
            {
                ...
                CellView cellView = ObjectPool.Spawn(cellViewPrefab);
    ...
    public void ClearSpawnedCells()
    {
        ...
        {
            CellView cellView = spawnedCells[i];
            if (cellView == null)
            {
                continue;
            }
    ...
    public CellView GetCellView(Vector3Int cube)
    {
        if (viewDic.TryGetValue(cube, out CellView cellView))
        {
            return cellView;
        }
        return null;
